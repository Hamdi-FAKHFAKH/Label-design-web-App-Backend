...
exports.GetlienProTypeAtelier = async (req, res) => {
	try {
	} catch (error) {
		res.status(400).json({
			Status: "LienProTypeAtelier Not Found",
...
exports.UpdateLienAtelierUAP = async (req, res) => {
	try {
		n = await LienProTypeAtelier.update(req.body, {
			where: { ProtypCod: req.params.id },
			returning: true,
		});
		res.status(301).json({
			Status: "Succes",
...
exports.DeleteLienAtelierUAP = async (req, res) => {
	try {
		n = await LienProTypeAtelier.destroy({
			where: { ProtypCod: req.params.id },
		}); // autre Méthode
